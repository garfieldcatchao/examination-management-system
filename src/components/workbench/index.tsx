...
function Workbench() {
  ...
  const [currentUser] = useState<UserInfo>({
    ...
    permissions: ['create_exam', 'manage_own_papers', 'view_own_students', 'grade_papers']
  });
  ...
  const [stats] = useState<StatItem[]>([
    ...
    { label: "待我阅卷", value: 12, icon: "fas fa-clipboard-check", trend: "-3" }
  ]);

  const [notifications, setNotifications] = useState<NotificationItem[]>([
    { id: '1', title: '考试提醒', message: '您创建的"计算机网络期末考试"将在30分钟后开始', type: 'warning', time: '2分钟前', read: false },
    { id: '2', title: '阅卷提醒', message: '计科2021-1班的"数据结构测验"有12份试卷待阅卷', type: 'warning', time: '1小时前', read: false },
    { id: '3', title: '学生提交', message: '软工2021-1班已有25名学生完成在线测试', type: 'info', time: '3小时前', read: true },
    { id: '4', title: '成绩统计', message: '您的"操作系统期中考试"成绩统计已生成', type: 'success', time: '5小时前', read: false },
    { id: '5', title: '班级通知', message: '计科2021-2班有新学生申请加入', type: 'info', time: '昨天', read: true }
  ]);
  ...
  const [todoList, setTodoList] = useState<TodoItem[]>([
    { id: '1', title: '期末考试阅卷 - 计算机网络', deadline: '今天 18:00', priority: 'high' },
    { id: '2', title: '下周考试安排确认', deadline: '明天 12:00', priority: 'medium' },
    { id: '3', title: '题库整理 - 操作系统章节', deadline: '本周内', priority: 'low' },
    { id: '4', title: '学生成绩统计分析', deadline: '周五前', priority: 'medium' },
  ...
  const popularQuestions = [
    { id: 1, title: 'TCP和UDP协议的区别', subject: '计算机网络', difficulty: '中等', usage: 23, creator: '张老师', isOwner: true },
    { id: 2, title: '进程和线程的区别', subject: '操作系统', difficulty: '中等', usage: 18, creator: '张老师', isOwner: true },
    { id: 3, title: '数据结构基本概念', subject: '数据结构', difficulty: '简单', usage: 15, creator: '张老师', isOwner: true },
    { id: 4, title: '计算机网络分层模型', subject: '计算机网络', difficulty: '困难', usage: 12, creator: '张老师', isOwner: true },
    { id: 5, title: '操作系统调度算法', subject: '操作系统', difficulty: '困难', usage: 10, creator: '张老师', isOwner: true }
  ];
  ...
  const canAccessStudentData = (classId: string) => {
    // 只能访问自己班级的学生数据
    return currentUser.classes.includes(classId) || currentUser.role === 'admin';
  };
    ...
    <div className="workbench">
      ...
      <header className="page-header">
        <div className="header-left">
          ...
          <p className="user-role-info">{getRoleDisplayName()} | {currentUser.department}</p>
        </div>
            ...
            {showNotifications && (
              <div className="notification-dropdown">
                <div className="notification-header">
                  <h4>通知中心</h4>
                  <button onClick={handleClearAllNotifications}>清空全部</button>
                </div>
          ...
          {stats.map((stat, index) => (
            <div key={index} className="stat-card">
              ...
              <div className="stat-info">
                ...
                {stat.trend && (
                  <div className={`stat-trend ${stat.trend.startsWith('+') ? 'positive' : stat.trend.startsWith('-') ? 'negative' : ''}`}>
                    {stat.trend}
                  </div>
                )}
              </div>
      ...
      <section className="quick-actions">
        ...
        <div className="actions-list">
          <button className="action-item" onClick={() => handleQuickAction('创建考试')}>
            <i className="fas fa-plus"></i>
            <span>创建考试</span>
          </button>
          <button className="action-item" onClick={() => handleQuickAction('管理试卷')}>
            <i className="fas fa-file-alt"></i>
            <span>管理试卷</span>
          </button>
          <button className="action-item" onClick={() => handleQuickAction('题库管理')}>
            <i className="fas fa-database"></i>
            <span>题库管理</span>
          </button>
          <button className="action-item" onClick={() => handleQuickAction('成绩管理')}>
            <i className="fas fa-chart-bar"></i>
            <span>成绩管理</span>
          </button>
          <button className="action-item" onClick={() => handleQuickAction('学生管理')}>
            <i className="fas fa-users"></i>
            <span>学生管理</span>
          </button>
          <button className="action-item" onClick={() => handleQuickAction('系统设置')}>
            <i className="fas fa-cog"></i>
            <span>系统设置</span>
          </button>
        </div>
